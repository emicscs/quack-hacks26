...
(function (global) {
    ...
    function playBlockedNavigationCue() {
        ...
        if (s && typeof s.playBlockedCue === "function") {
            s.playBlockedCue();
        }
    }
    ...
    function resolveNextX(delta) {
        ...
        var directionBound = getBoundForDirection(direction);
        if (directionBound !== null && ((direction < 0 && candidate < directionBound) || (direction > 0 && candidate > directionBound))) {
            if (currentX === directionBound) return null;
            candidate = directionBound;
        }
        var firstY = getYAt(candidate);
        if (!isNaN(firstY)) return candidate;
        var probeStep = Math.max(Math.abs(delta) * 0.5, step);
        for (var i = 1; i <= NAV_PROBE_STEPS; i++) {
            var probeX = candidate + direction * probeStep * i;
            if (directionBound !== null && ((direction < 0 && probeX < directionBound) || (direction > 0 && probeX > directionBound))) {
                break;
            }
            var probeY = getYAt(probeX);
            if (!isNaN(probeY)) return probeX;
        }
        return null;
    }
    ...
    function panWindowIfNeeded(direction) {
        ...
        var edgePadding = width * PAN_EDGE_RATIO;
        var shouldShiftRight = direction > 0 && currentX >= (domain.max - edgePadding);
        var shouldShiftLeft = direction < 0 && currentX <= (domain.min + edgePadding);
        if (!shouldShiftLeft && !shouldShiftRight) return false;
    ...
    function init() {
        setFunction(document.getElementById("function-catalog").value || "sin(x)", { preserveView: false });
        notifySettingsChange();
